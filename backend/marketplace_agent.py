...
──────────────────────────────────────────
Playwright + Gemini ile Facebook Marketplace'te serbest metin araması yapar.
Ürün başlığı, fiyat, açıklama, konum ve link bilgilerini döndürür.
browser-use kaldırıldı; Render ve cloud ortamlarında chromium ile çalışır.
"""
import asyncio
import json
import os
...
def _parse_marketplace_result(text_or_data: str | dict, query: str) -> dict:
    """Agent/extractor çıktısını parse eder ve arama terimine göre filtreler."""
    empty = {
        "listings": [],
        "total_found": 0,
    }
...
async def search_marketplace_listings(
    ...
    try:
        with _langsmith_env():
            from browser.playwright_marketplace import load_marketplace_page
            from browser.marketplace_extractor import extract_listings_from_html

            html = await load_marketplace_page(search_url, scroll_count=10, scroll_delay_ms=800)
            logger.info("Marketplace page loaded, HTML length: %d", len(html))

            extracted = await extract_listings_from_html(html, query)
            parsed = _parse_marketplace_result(extracted, query)
        ...
        logger.error("Marketplace search error: %s", str(e), exc_info=True)
        error_result = {
            "listings": [],
            "total_found": 0,
            "error": str(e),
